...
export const AITaskFlow: React.FC<AITaskFlowProps> = ({ agentId }) => {
  ...
  return (
    <div className="w-full rounded-2xl bg-[#FFFFFF] border border-[#E8DFF0] shadow-xl p-6 md:p-8 text-[#1B1024]">
      {/* Top Controls */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-[#E8DFF0]">
        <div>
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase bg-[#FCE7F3] text-[#E11D72] mb-1">
            <Sparkles className="w-3 h-3" /> Autonomous Reasoning Engine
          </span>
          <h3 className="text-base font-bold font-sora text-[#2B0D3A]">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-[#F8F7FA] border border-[#E8DFF0] text-[#6F6078] hover:text-[#2B0D3A]"
          >
            ...
            <div
              ...
              className={cn(
                ...
                isCurrent
                  ...
                  : isCompleted
                  ? "bg-[#F8F7FA] border-[#2B0D3A]/40"
                  : "bg-[#FFFFFF] border-[#E8DFF0] opacity-60 hover:opacity-100"
              )}
            >
              ...
              <span className="text-[10px] text-[#6F6078] font-semibold uppercase tracking-wider">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="p-5 rounded-2xl bg-gradient-to-r from-[#F8F7FA] to-[#F4EEFF] border border-[#E8DFF0] flex flex-col md:flex-row md:items-center justify-between gap-4"
        >
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-[#E11D72] uppercase tracking-wider font-sora">
                {steps[activeStep].title} Stage Analysis
              </span>
              <span className="text-[11px] text-[#6F6078]">| Elapsed Time: {(activeStep + 1) * 120}ms</span>
            </div>
            <p className="text-xs md:text-sm text-[#1B1024] font-medium leading-relaxed">
              {steps[activeStep].desc}
            </p>
          </div>

          <div className="shrink-0 flex items-center gap-2">
            <span className="text-xs font-bold text-[#2B0D3A] px-3 py-1.5 rounded-lg bg-white border border-[#E8DFF0] shadow-2xs">
